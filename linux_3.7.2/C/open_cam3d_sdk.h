...
extern "C"
{
	...
	//输出参数：out_status_json（输出设置状态）、out_size（输出文本大小）、maxnum(曝光数，填入DfCaptureData接口)
	//返回值： 类型（int）:返回0表示设置参数成功;否则失败。
	...
	//函数名： DfSavePointcloudToPcd
	//功能： 保存pcd点云
	//输入参数：pointcloud(点云)、brightness（亮度图）、channels（亮度图通道数）、path(路径)
	//输出参数：gain(亮度图增益)
	//返回值： 类型（int）:返回0表示设置参数成功;否则失败。
	DF_SDK_API int DfSavePointcloudToPcd(float* pointcloud, unsigned char* brightness, int channels, const char* path);
